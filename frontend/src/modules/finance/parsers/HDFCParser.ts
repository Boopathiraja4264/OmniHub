...
function tryParseDate(raw: string): string {
  ...
  // DD/MM/YY — HDFC uses 2-digit year (e.g. 01/01/26)
...
// Date: DD/MM/YY (2-digit year). Each transaction spans multiple pdfjs lines.
// Value Dt column also has a DD/MM/YY date but is at a larger x — tokens[0] (leftmost) is always the Date column.
export async function parseHDFCPDF(file: File): Promise<ParsedRow[]> {
  const { lines } = await extractLines(file);

  let withdrawalX = -1, depositX = -1, inTable = false;
  for (const line of lines) {
    const text = line.map(i => i.text).join(' ');
    // Header row contains both "Withdrawal" and "Deposit" column labels
    if (/Withdrawal/i.test(text) && /Deposit/i.test(text) && /Narration|Closing/i.test(text)) {
    ...
    if (/STATEMENT SUMMARY/i.test(text)) { inTable = false; continue; }
    if (!inTable) continue;
  }

  // Reset for transaction pass
  inTable = false;
  let currentDate = '';
  let currentRawDate = '';
  ...
  for (const line of lines) {
    ...
    if (/STATEMENT SUMMARY/i.test(text)) { inTable = false; currentDate = ''; continue; }
    if (!inTable) continue;
    ...
    if (withdrawalX >= 0 && depositX >= 0) {
      ...
      type = /\bDR[-\s]|\bNEFT DR\b|\bIMPS-|\bNWD-|\bATW-/i.test(currentNarration) ? 'EXPENSE' : 'INCOME';
    }
